...
$.SvgCanvas = function (container, config) {
    ...
    var runExtensions = this.runExtensions = function (action, vars, returnArray) {
        ...
        //GOL: synoptic's objects management in synoptic.editor.main.js.
        managementSynopticObject(action, vars);
        ////////////////////////////////////////////////////////////////
    ...
    //GOL: functions to enable the copy of a custom synoptic element.
    var copyCustomElem = function (el) {
        // manually create a copy of the element
        var new_el = document.createElementNS(el.namespaceURI, el.nodeName);

        // set the copied element's new id
        if ($(el).attr("type_name").length > 0) {
            new_el.removeAttribute("id");

            var newId = $(el).attr("type_name") + "_" + getNextId().split("_")[1];
            //check newId if exists
            while ($("#" + newId).length == 1) {
                newId = $(el).attr("type_name") + "_" + getNextId().split("_")[1];
            }
            new_el.id = newId;

            new_el.setAttribute("type_name", $(el).attr("type_name"));
            new_el.setAttribute("type_custom", "1");
            //new_el.setAttribute("class", $(el).attr("class"));
            new_el.setAttribute("class", "");
        ...
        switch ($(el).attr("type_name")) {
            case "multiLineCounters":
                var clone = $(el).clone();

                //reset object content
                clone.children().first().children().text("");//clean label
                clone.children().last().children().first().children().text("");//clean labels from lineStatus                
                clone.children().attr("class", " ");//clean lineCounter class
                clone.children().last().children().first().attr("class", " ");//clean lineCounter light class
                clone.children().last().children().last().attr("class", " ");//clean lineStatus class

                $(new_el).append(clone.children().first());
                $(new_el).append(clone.children().last());
            ...
            default:
                ...
                $.each(el.childNodes, function (i, child) {
                    switch (child.nodeType) {
                        case 1: // element node
                            new_el.appendChild(copyCustomChildElem(child));
                            break;
                        case 3: // text node
                            //new_el.textContent = child.nodeValue;
                            break;
                ...
                $(new_el).children().text("");

                return new_el;
        }   
    };

    var copyCustomChildElem = function (el) {
        // manually create a copy of the element
        var new_el = document.createElementNS(el.namespaceURI, el.nodeName);
        $.each(el.attributes, function (i, attr) {
            if (attr.localName != '-moz-math-font-style') {
    ...
    (function () {
        ...
        var mouseDown = function (evt) {
            ...
            switch (current_mode) {
                ...
                case 'fhpath':
                    ...
                    addSvgElementFromJson({
                        ...
                        attr: {
                            ...
                            id: getNextId(),
                            fill: 'none',
                            'type_custom': 0,//GOL: added custom attr to identify a no synoptic's object.
                            opacity: cur_shape.opacity / 2,
                ...
                case 'rotate':
                    ...
                    break;
                case "custom":
                    //GOL: mousedown event during object creation.
                    break;
                default:
        ...
        var mouseMove = function (evt) {
            ...
            switch (current_mode) {
                ...
                case 'line':
                    ...
                    break;
                case "custom":
                    //GOL: mousemove event during object creation.
                    break;
                case 'foreignObject':
                    // fall through
                case 'square':
        ...
        var mouseUp = function (evt) {
            ...
            switch (current_mode) {
                ...
                case 'rotate':
                    ...
                    break;
                case "custom":
                    //GOL: mouseUp event during object creation.
                    break;
                default:
                    // This could occur in an extension
                    break;
            ...
            if (!keep && element != null) {
                ...
                //GOL: if a svg's object is not created return to prevent an exception.
        ...
        var dblClick = function (evt) {
            ...
            //GOL: disable svgEditor's dblclick on synoptic's objects.
            if ($(parent).attr("type_custom") == 1)
                return;
            //////////////////////////////////////////////////////////
    ...
    this.setSvgString = function (xmlString) {
        try {
            ...
            //GOL: comment prepareSvg method because it clean all custom attributes.
            //this.prepareSvg(newDoc);//ORIGINAL
    ...
    //GOL: custom function to insert custom SVg from file.svg.
    this.importCustomSvg = function (xmlString) {
        var j, ts;
        try {
            // Get unique ID
            var uid = svgedit.utilities.encode64(xmlString.length + xmlString).substr(0, 32);
            ...
            var newDoc = svgedit.utilities.text2xml(xmlString);
            var type_name = $(newDoc).find('svg').attr("type_name");


            //this.prepareSvg(newDoc); //IE crash with custom svg object
            ...
            var group = svgdoc.createElementNS(NS.SVG, "g");
            ...
            for (i = 0; i < attrs.length; i++) {
                var attr = attrs[i];
                group.setAttribute(attr.nodeName, attr.nodeValue);
            }

            var objId = type_name + getNextId().substring(3);

            //change objId if exists
            while ($("#" + objId).length == 1) {
                objId = type_name + getNextId().substring(3);
            }

            group.id = type_name + getNextId().substring(3);
            group.setAttribute("type_name", type_name);
            group.setAttribute("type_custom", "1");
            ...
            if (isMSIEBrowser()) { $("#tool_make_link").html('<img class="svg_icon" src="images/properties.svg" width="24px" height="24px">') };
    ...
    this.pasteElements = function (type, x, y) {
        ...
        while (len--) {
            ...
            //var copy = copyElem(elem);//ORIGINAL
            //GOL: add copy for custom object.
            var isCustomElem = ($(elem).attr("type_custom") == 1) ? true : false;
            var copy = (isCustomElem) ? copyCustomElem(elem) : copyElem(elem);
            //////////////////////////////////

...
function managementSynopticObject(action, vars) {
    ...
    switch (action) {
        case "mouseDown":
            if (vars.selectedElements[0] != null && vars.selectedElements[0] != 'undefined')
    ...
    if (id_selected.length > 0) {
        ...
        if (id_selected.attr('type_custom') == "1") {
            $("#tool_make_link").html('<img class="svg_icon" src="images/properties.svg" width="24px" height="24px">')
            $("#tool_make_link").show();
...
function saveManagement() {
    ...
    if (currentSinottico.length > 0) {//save
...
function dialogAreaSelection() {
    ...
    $("#dialog_container").css("margin-left", -parseInt($("#dialog_container").width()) / 2);
    $("#dialog_container").css("margin-top", -parseInt($("#dialog_container").height()) / 2);
    $('#dialog_content').addClass("dialogContent-areaSelection");

    //insert messagge area on top
    $('<div id="dialoxBox-message">Select the plant area.</div>').insertBefore("#dialog_content");

    //content HTML
    var html = '<div>';
    html += '<ul id="loader"><li>&nbsp;</li><li><img src="images/loader.gif" height="20" width="20"></li><li>&nbsp;</li></ul>';
    ...
    if (areaList.length == 0) {
        ...
        getPlantTree(null, function (data) {
            ...
            if (data.ErrorList.length == 0) {
                ...
                $('#tools_left').append('<div id="synopticObjList" class="tool_button" title="MOM Synoptic Object List">MOM LIB</div>');
                $('#synopticObjList').click(dialogMOMLibrary);

                //set saveButtons' icon
                $('#tool_save div').html('<img src="images/save.png" height="24" width="24">');
                $('#tool_saveAs div').html('<img src="images/saveAs.png" height="26" width="26" Style="margin-left: -2px;">');
                ...
                $('#dialoxBox-message').fadeOut(function () {
    ...
    function populateSelect() {
        ...
        $.each(areaList, function () {
            $('#areaList').append('<option value="' + this.EquipmentId + '">' + this.EquipmentLongDescription + '</option>');
        });
...
function dialogMOMLibrary(e) {
    ...
    //insert messagge area on top
    $('<div id="dialoxBox-message"> Select the synoptic object to insert </div>').insertBefore("#dialog_content");

    //set dialog content
    var html = '<div id="synoptic-btnList" style="width: 100%; text-align: center;">';
    ...
    //html += addSynopticObj("Counter", "Counter", false, true, true);
    ...
    html += addSynopticObj("Status List Service Connector", "StatusListServiceConnector", false, true, false);
    html += addSynopticObj("Status Thread", "StatusThread", false, true, false);
    html += addSynopticObj("Status CapStation", "StatusCapStation", false, true, false);
    html += addSynopticObj("Status CapStation Service Connector", "StatusCapStationServiceConnector", false, true, false);
    html += addSynopticObj("Status Seats", "StatusSeats", false, true, false);
    html += addSynopticObj("Status Support Connector", "StatusSupportConnector", false, true, false);
    html += addSynopticObj("Status CpStation", "StatusCpStation", false, true, false);
    html += addSynopticObj("Status Camera", "StatusCamera", false, true, false);
    html += addSynopticObj("Status Camera Service Connector", "StatusCameraServiceConnector", false, true, false);
    html += addSynopticObj("Status TanksBumper", "StatusTanksBumper", false, true, false);
    html += addSynopticObj("Status Extemporary", "StatusExtemporary", false, true, false);
    html += addSynopticObj("Status Extemporary OP", "StatusExtemporaryOP", false, true, false); 
    html += addSynopticObj("Status PickToLight", "StatusPickToLight", false, true, false);
    html += addSynopticObj("Status Sniffer", "StatusSniffer", false, true, false);
    html += addSynopticObj("Status SetPoint", "StatusSetPoint", false, true, false);
    html += addSynopticObj("Sequence Alarm", "SequenceAlarm", false, true, false);
    html += addSynopticObj("Status Dashboard", "StatusDashboard", false, true, false);
    html += addSynopticObj("Battery Pack", "BatteryPack", false, true, false);
    html += addSynopticObj("Battery Pack Line", "BatteryPackLine", false, true, false);
    html += addSynopticObj("Station Battery Pack", "Station", false, true, false);
    html += addSynopticObj("Counter Battery Pack", "BatteryPackCounter", false, true, false);
    html += addSynopticObj("Andon", "Andon", false, true, false);
    html += addSynopticObj("OT", "OT", false, true, false);
    html += addSynopticObj("BatteryPackAGV", "BatteryPackAGV", false, true, false);
    ...
    html += '<div id="svgPreview" style="display: block; position: fixed; bottom: 0; right: 0; border: dotted black 1px; background: white;"><div id="area"></div></div>';
    $('#dialog_content').html(html);
    ...
    function addSynopticObj(objName, fileName, isSmall, isMedium, isLarge) {
        ...
        var row = '<ul style="padding: 0; margin: 0; width: 100%; list-style: none;">';
        ...
        row += '<li class="svgObjLabel" data-previewFileName="' + fileName + '_Medium" style="float: left; width: 35%; text-align: left;">&nbsp;' + objName + '</li>';
        row += '<li style="float: left; width: 20%;"><button data-fileName="' + fileName + '_Small" style="width: 80%; visibility:' + ((isSmall) ? "visible" : "hidden") + ';">Small</button></li>';
        row += '<li style="float: left; width: 20%;"><button data-fileName="' + fileName + '_Medium" style="width: 80%; visibility:' + ((isMedium) ? "visible" : "hidden") + ';">Medium</button></li>';
        row += '<li style="float: left; width: 20%;"><button data-fileName="' + fileName + '_Large" style="width: 80%; visibility:' + ((isLarge) ? "visible" : "hidden") + ';">Large</button></li>';
        row += '</ul>';
                    ...
                    button.click(function () {
                        ...
                        switch (fileName.split("_")[0]) {
                            ...
                            case "Cap":
                                ...
                                break;
                            default:
                                _typeStationSelected = (fileName.indexOf('Large') >= 0) ? 'LARGE' : 'MEDIUM';
    ...
    function setPreviewOnObjLabel() {
        ...
        $(".svgObjLabel").mouseover(function (e) {
            ...
            $.get(location.origin + _global.pathCustomSvgObj + fileName, function (data) {

                var svgObj = $.parseXML(data);

                //set svg tag                                    
                $(svgObj).find('svg').removeAttr('viewBox');
                $(svgObj).find('svg').attr("viewBox", "0 0 " + $(svgObj).find('svg').attr("width") + " " + $(svgObj).find('svg').attr("height"));
                $(svgObj).find('svg').attr("width", $(svgObj).find('svg').attr("width"));
                $(svgObj).find('svg').attr("height", $(svgObj).find('svg').attr("height"));
...
function dialogClear(callback) {
    ...
    //insert messagge area on top
    $('<div id="dialoxBox-message">! WARNING !</div>').insertBefore("#dialog_content");

    //set dialog content	
    var html = "<div>Si è sicuri di voler aprire una nuova area di lavoro dell'editor?</div>";
    $('#dialog_content').html(html);
...
function dialogSaveAs() {
    ...
    //insert messagge area on top
    $('<div id="dialoxBox-message">Save synoptic.</div>').insertBefore("#dialog_content");
...
function dialogLoad() {
    ...
    //insert messagge area on top
    $('<div id="dialoxBox-message">Select a synoptic.</div>').insertBefore("#dialog_content");
        ...
        if (data.ErrorList.length == 0) {
            ...
            if (synopticList.length == 0) {
                ...
                $.each(synopticList, function () {
                    $('#synopticList').append('<option value="' + this.Name + '">' + this.Name + '</option>');
                });
...
function dialogSynopticObjectProperties() {
    ...
    switch (synopticObjTypeName) {
        case "Andon":
            var andonIdOld = $('#' + synopticObjId).children('rect')[0].attributes['andonId'].value;
            var andonId = prompt("Inserisci l'id dell' Andon", andonIdOld);
            if (andonId != undefined && andonId != '') {
                $('#' + synopticObjId).children('rect')[0].attributes['andonId'].value = andonId;
            }
            return;
        case "OT":
            var otIdOld = $('#' + synopticObjId).children('rect')[0].attributes['otId'].value;
            var otId = prompt("Inserisci l'id dell' OT", otIdOld);
            if (otId != undefined && otId != '') {
                $('#' + synopticObjId).children('rect')[0].attributes['otId'].value = otId;
            }
            return;
    }

    //insert messagge area on top
    $('<div id="dialoxBox-message">Associate the synoptic object "' + synopticObjTypeName + '"</div>').insertBefore("#dialog_content");

    //add scripts
    var scripts = '<script type="text/javascript" src="jquery-1.11.1.min.js"></script>';
    scripts += '<script type="text/javascript" src="synopticEditor/scripts/jsTree/jstree.min.js"></script>';
    ...
    $("#dialogBox-ok").click(function () {
        ...
        switch ($(jsTreeNodeSelector).length) {
            ...
            case 1: //single element selected
                ...
                var equipmentPath = getEquipmentPathFromJsTreeNode(jsTreeNodeSelector, plantLevel);
                    ...
                    case "station":
                        ...
                        $.each(jsTreeNodeList, function () {
                            ...
                            if (plantLevel != "5") {
                                ...
                                $('#dialoxBox-message').fadeOut(function () {
                                    $(this).html("! multiple insertion enable only for stations !");
                                }).fadeIn();
                                return false; //each's break
                            }
                            else {

                                equipmentPathList.push(getEquipmentPathFromJsTreeNode(this, plantLevel));
                                descriptionList.push($(this).text().replace(")", "").split(' ')[0]);
                            }
                        });

                        if (equipmentPathList.length == jsTreeNodeList.length && descriptionList.length == jsTreeNodeList.length) {
                            ...
                            addMultipleStation(descriptionList, equipmentPathList, startIdx, endIdx, stationPlacing, stationSorting);
                            closeDialog();
                            break;
                        }
                        else { return; }
    ...
    function getJsTreeHTML() {
        ...
        //tree html
        var html = '<div id="jstree">';

        $.each(areaList, function (i, area) {

            //if (area.EquipmentId == currentAreaId) {
            html += '<ul><li data-plantLevel="' + area.Level + '" data-equipmentId="' + area.EquipmentId + '">' + area.EquipmentDescription;
            ...
            html += '<ul>';
            $.each(area.Children, function (i, lineGroup) {

                html += '<li data-plantLevel="' + lineGroup.Level + '" data-equipmentId="' + lineGroup.EquipmentId + '">' + lineGroup.EquipmentDescription;

                if (lineGroup.Children == undefined)
                    return true; //each's continue                    

                html += '<ul>';
                $.each(lineGroup.Children, function (i, line) {
                    switch (line.Level) {
                        case 3:
                            html += '<li data-plantLevel="' + line.Level + '" data-equipmentId="' + line.ObjectTypeId + 'x' + line.EquipmentId + '">' + line.EquipmentDescription + '</li>';
                            break;
                        case 4:
                            if (synopticObjTypeName != "StatusExtemporaryOP")
                                html += '<li data-plantLevel="' + line.Level + '" data-equipmentId="' + line.ObjectTypeId + 'x' + line.EquipmentId + '" data-equipmentPath="' + line.EquipmentPath + '" data-description="' + line.EquipmentPathDescription + '">' + line.EquipmentDescription + '</li>';
                            break;
                        case 5:
                            if (synopticObjTypeName == "StatusExtemporaryOP")
                                html += '<li data-plantLevel="' + line.Level + '" data-equipmentId="' + line.ParentId + 'x' + line.ObjectId + '" data-equipmentPath="' + line.EquipmentPath + '" data-description="' + line.EquipmentPathDescription + '">' + line.EquipmentDescription + '</li>';
                            break;
                        default:
                            html += '<li data-plantLevel="' + line.Level + '" data-equipmentId="' + line.EquipmentId + '">' + line.EquipmentDescription + '</li>';
                            break;
                    }
                    ...
                    //    case "Andon":
                    //        //management an empty node
                    //        if (line.Andons == undefined)
                    //            return true; //each's continue

                    //        html += '<ul>';
                    //        $.each(line.Andons, function (i, andon) {
                    //            html += '<li data-plantLevel="' + line.Level + '" data-equipmentId="' + andon.AndonId + '">' + andon.AndonDescription + '</li>';
                    //        });
                    //        html += '</ul>';
                    //        break;
                    //    case "StatusThread":
                    //        //management an empty node
                    //        if (line.Accumulators == undefined)
                    //            return true; //each's continue

                    //        html += '<ul>';
                    //        $.each(line.Accumulators, function (i, accumulator) {
                    //            html += '<li data-plantLevel="' + line.Level + '" data-equipmentId="' + accumulator.AccumulatorId + '">' + accumulator.AccumulatorDescription + '</li>';
                    //        });
                    //        html += '</ul>';
                    //        break;
                    //    default:
    ...
    function getStationOptionsHTML() {
        ...
        $('#dialog_content').removeClass("dialogContent-plantSize").addClass("dialogContent-plantSize-station");
    ...
    function getLineProgressOptionsHTML() {
        ...
        $('#dialog_content').removeClass("dialogContent-plantSize").addClass("dialogContent-plantSize-lineProgressBar");
            ...
            case "3"://Thread
                ...
                switch (synopticObjTypeName) {
                    ...
                    case "StatusDashboard":
                        ...
                        var areaId = $(nodeSelector).parent().parent().attr("data-equipmentId");
                    ...
                    case "StatusThread":
                        ...
                        if ($(nodeSelector).parent().parent().attr("data-plantLevel") === "2") {

                            var Id = $(nodeSelector).attr("data-equipmentId");
                            var lineId = $(nodeSelector).parent().parent().attr("data-equipmentId");
                            var areaId = $(nodeSelector).parent().parent().parent().parent().attr("data-equipmentId");
    ...
    function setSynopticObj(id, nodeSelector, equipmentPath) {
        ...
        switch (synopticObjTypeName) {
            ...
            case "StatusList":
            case "StatusListSC":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " statuslist");
                break;
            case "Status":
            case "LineStatus"://deprecated
            ...
            case "StatusThread"://deprecated
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " statusthread");
                $("#" + id).children("text").text(short_desc[short_desc.length - 1]);
                break;
            case "lineCounter":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " lineCounter");
                $("#" + id).children("text").text(short_desc[short_desc.length - 1]);
                break;
            case "LineCounterProgressBar":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " lineCounterProgressBar");
                $("#" + id).children("text").text(short_desc[short_desc.length - 1]);
                break;
            case "StatusCapStation":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " capStation");
                $("#" + id).children("text").text(short_desc[short_desc.length - 1]);
                break;
            case "StatusCamera":
                ...
                break;
            case "StatusTanksBumper":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " tanksBumper");
                $("#" + id).children("text").text(short_desc[short_desc.length - 1]);
                break;
            case "StatusSeats":
                ...
                break;
            case "StatusSupportConnector":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " suppConnect");
                $("#" + id).children("text").text(short_desc[short_desc.length - 1]);
                break;
            case "StatusCpStation":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " statusCpStation");
                break;
            case "StatusExtemporary":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " extemporary");
                $("#" + id).children("text").text(short_desc[short_desc.length - 1]);
                break;
            case "StatusExtemporaryOP":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " extemporaryOP");
                break;
            case "StatusPickToLight":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " pickToLight");
                $("#" + id).children("text").text(short_desc[short_desc.length - 1]);
                break;
            case "StatusSniffer":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " sniffer");
                $("#" + id).children("text").text(short_desc[short_desc.length - 1]);
                break;
            case "StatusSetPoint":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " setPoint");
                $("#" + id).children("text").text(short_desc[short_desc.length - 1]);
                break;
            case "counter":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " counter");

                switch (plantLevel) {
                    case "4":
                        $("#" + id).children('text[class="desc"]').text(short_desc[short_desc.length - 1]);
                        break;
                    case "5":
                        $("#" + id).children('text[class="desc"]').text(short_desc[0]);
                        break;
                }
                break;
            case "SequenceAlarm":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " sequenceAlarm");
                break;
            case "StatusDashboard":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " statusDashboard");
                break;
            case "BatteryPack":
                var _desc = $(nodeSelector).attr("data-description");
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " batteryPack");
                $("#" + id).children('text[class="bin"]').text(_desc);
                break;
            case "StationBatteryPack":
                var _desc = $(nodeSelector).attr("data-description");
                $("#" + id).children('rect').attr('fill', 'url(#grad_' + equipmentPath + ')');
                $("#" + id).children(0).children(0)[0].id = 'grad_' + equipmentPath;
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " stationBatteryPack");
                $("#" + id).children('text[class="value"]').text(_desc);
                break;
            case "BatteryPackCounter":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " counterBatteryPack");
                $("#" + id).children('text[class="value"]').text('##');
                break;
            case "BatteryPackAGV":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " agvBatteryPack");
                $("#" + id).children('text[class="partNumber"]').text('');
                $("#" + id).children('text[class="agvTitle"]').text('');
                break;
            case "StopLightIn":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " stopLightIn");
                break;
            case "StopLightOut":
                $("#" + id).attr("class", "equipmentPath_" + equipmentPath + " stopLightOut");
                break;
        }
    }

    //insert multiple station management
    function addMultipleStation(stationDescriptionList, stationEquipmentPathList, currentStationIdx, lastStationidx, placing, sorting) {

        var i = currentStationIdx;

        //get synoptic station object info
        var synopticObjId = $('#elem_id').val();
        var synopticObjPosX = $("#" + synopticObjId).children("rect").attr("x");
        var synopticObjPosY = $("#" + synopticObjId).children("rect").attr("y");

        //set synoptic station object
        var equipmentPath = stationEquipmentPathList[i];
        var stationDescription = stationDescriptionList[i];
        $("#" + synopticObjId).attr("class", "equipmentPath_" + equipmentPath + " station status");
        $("#" + synopticObjId).children("text").text(stationDescription);

        //check station description length to reduce fonte size
        if (stationDescription.length > 8) {
            if (_typeStationSelected == "LARGE")
                $("#" + synopticObjId).children('text[class="desc"]').attr("font-size", "28");
            else
                $("#" + synopticObjId).children('text[class="desc"]').attr("font-size", "14");
        }
        else if (stationDescription.length > 5) {
            if (_typeStationSelected == "LARGE")
                $("#" + synopticObjId).children('text[class="desc"]').attr("font-size", "25");
            else
                $("#" + synopticObjId).children('text[class="desc"]').attr("font-size", "15");
        }
        else {
            if (_typeStationSelected == "LARGE")
                $("#" + synopticObjId).children('text[class="desc"]').attr("font-size", "35");
            else
                $("#" + synopticObjId).children('text[class="desc"]').attr("font-size", "20");
        }

        //init text values
        $("#" + synopticObjId).children('text[class="text"]').text("-");
        $("#" + synopticObjId).children('text[class="value"]').text("0");
        ...
        $.get(url + fileName, function (data) {
            ...
            switch (placing) {
                case "0"://horizontal
                    $("#selected_x").val(synopticObjPosX + offset).trigger("change");
                    break;
                case "1"://vertical
                    $("#selected_y").val(synopticObjPosY + offset).trigger("change");
                    break;
            }
            ...
            addMultipleStation(stationDescriptionList, stationEquipmentPathList, i, lastStationidx, placing, sorting);
            ...
            switch (synopticObjTypeName) {
                ...
                case "StatusThread":
                    ...
                    $.each(jsTreePath, function (i) {
                        ...
                        $("#jstree").find('[data-equipmentid="' + this + '"] .jstree-icon.jstree-ocl').trigger("click");

                        //for the last node select the node
                        if (i == jsTreePath.length - 1) {

                            $("#jstree").find('[data-equipmentid="' + this + '"]').attr("aria-selected", "true");
                            $("#jstree").find('[data-equipmentid="' + this + '"]').children("a.jstree-anchor").addClass("jstree-clicked");
                ...
                case "StopLightOut":
                    ...
                    for (var i = 1; i < jsTreePath.length; i++) {
                        ...
                        if (i == jsTreePath.length - 1) {
                            ...
                            //$("#jstree").find('[data-equipmentid="' + jsTreePath[i] + '"] .jstree-icon.jstree-ocl').trigger("click");
                            $("#jstree").find('[data-equipmentid="' + jsTreePath[i - 1] + '"]').find(".jstree-children").children('[data-equipmentid="' + jsTreePath[i] + '"]').attr("aria-selected", "true");
                            $("#jstree").find('[data-equipmentid="' + jsTreePath[i - 1] + '"]').find(".jstree-children").children('[data-equipmentid="' + jsTreePath[i] + '"]').find("a.jstree-anchor").addClass("jstree-clicked");
                ...
                default:
                    ...
                    for (var i = 0; i < jsTreePath.length; i++) {
                        ...
                        if (i == jsTreePath.length - 2) {
                            ...
                            //$("#jstree").find('[data-equipmentid="' + jsTreePath[i] + '"] .jstree-icon.jstree-ocl').trigger("click");
                            $("#jstree").find('[data-equipmentid="' + jsTreePath[i] + '"]').find(".jstree-children").children('[data-equipmentid="' + jsTreePath[i + 1] + '"]').attr("aria-selected", "true");
                            $("#jstree").find('[data-equipmentid="' + jsTreePath[i] + '"]').find(".jstree-children").children('[data-equipmentid="' + jsTreePath[i + 1] + '"]').find("a.jstree-anchor").addClass("jstree-clicked");
    ...
    function setLineProgressBarGraphic(id) {
        ...
        $("#" + id).children("rect").attr("width", widthLinepb);
    }
}
...
function dialogInformation(title, messageContent) {
    ...
    //insert messagge area on top
    $('<div id="dialoxBox-message">Select the transitStation to associate.</div>').insertBefore("#dialog_content");
...
function wrapString(str, maxRowLength) {
    ...
    if (maxRowLength < maxWordLength)
        maxRowLength = maxWordLength;
...
function getPlantTree(param, callback) {
    ...
    $.ajax({
        ...
        error: function (xhr, status, error) {
            ...
            var result = {
                ErrorList: [{
                    Description: "getPlantTree()  [Status: " + status + " || Error: " + error + "]"
                }]
            };
            callback(result);
        }
...
function insertSynoptic(param, callback) {
    ...
    $.ajax({
        ...
        error: function (xhr, status, error) {
            ...
            var result = {
                ErrorList: [{
                    Description: "saveSynoptic()  [Status: " + status + " || Error: " + error + "]"
                }]
            };
            callback(result);
        }
...
function getSynoptic(param, callback) {
    ...
    $.ajax({
        ...
        error: function (xhr, status, error) {
            ...
            var result = {
                ErrorList: [{
                    Description: "getSynoptic()  [Status: " + status + " || Error: " + error + "]"
                }]
            };
            callback(result);
        }
...
function ajaxErrorFormatDescription(apiMethod, jqXHR, textStatus, errorThrown) {
    ...
    var str = "";
    str += "" + "\n";
    return "WebInterface - " + apiMethod + "[Status: " + textStatus + " || Error: " + errorThrown + "]";
}//work in progress...